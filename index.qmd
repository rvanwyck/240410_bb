...
![Artwork by Allison Horst](240410_brown_bag_files/allison-horst-git-workflow%20(1).png){alt="Artwork by Allison Horst"}
...
```{r} getwd()  ## If the result is not what you want reset your working directory ## setwd("X:/YOUR PREFERED WORKING DIRECTORY")}
```
...
```{r} options(   usethis.description = list(     `Authors@R` = 'person("Jane", "Doe", email = "jane@example.com")'   ) )}
```
...
```{r} ## git config --global user.name "my_user_name"}
```
...
```{r} ## git config --global user.email "my_email@alaska.edu"}
```
...
```{r} ## git config --global credential.helper 'cache --timeout=10000000'}
```
...
```{r} ## git config --global init.defaultBranch main}
```
